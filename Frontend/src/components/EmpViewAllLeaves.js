...
//search data according to the Name and date

filterData(GetAllLeavesEmp,searchKey){
    const result =GetAllLeavesEmp.filter((LeaveData)=>
   
    LeaveData.Today.includes(searchKey)||
    LeaveData.Emp_Name.toLowerCase().includes(searchKey) ||
    LeaveData.Emp_Name.includes(searchKey)
    )
    ...
    return (
        <div>
              ...
              <div style={{height:'80px', backgroundColor:"#FA9c1B", marginTop:'-20px'}}>
                ...
                <h1 style={{color:'#Black', textAlign:'center',fontSize:"60px"}}>LEAVE FORMS LIST</h1>
